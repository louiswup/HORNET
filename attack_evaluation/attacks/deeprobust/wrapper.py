...
def deeprobust_wrapper(attack: type[BaseAttack],
    ...
    if target_model == None:
        ...
        attack = attack(model=DeepRobustModel(model), device=inputs.device.type, target_model = target_model)
...
class DeepRobustMinimalWrapper:
    def __init__(self, model: DeepRobustModel, device: str, attack: type[BaseAttack], init_eps: float,
            search_steps: int, max_eps: Optional[float] = None, target_model: Optional[nn.Module] = None, batched: bool = False):
        self.model = model
    ...
    def generate(self, image: Tensor, label: Tensor, **attack_params) -> Tensor:
        ...
        for i in range(self.search_steps):
            if self.batched:
                adv_inputs_run = self.attack.generate(image=image, label=label, epsilon=batch_view(eps),
                                                      **attack_params)
            else:
            ...
            if self.target_model == None:
                ...
                logits = self.target_model(adv_inputs_run)
