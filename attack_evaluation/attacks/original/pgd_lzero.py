...
class BinarySearchMinimalWrapperL0:
    def __init__(self, model: nn.Module, attack: partial, init_eps: float, search_steps: int,
        ...
        self.max_eps = 1023#wjp
        #self.max_eps = max_eps#wjp
        self.targeted = False
    ...
    def __call__(self, inputs: Tensor, labels: Tensor, target_model: Optional[nn.Module]=None) -> Tensor:
        ...
        best_eps = torch.full_like(eps_low, torch.iinfo(torch.long).max if self.max_eps is None else self.max_eps)
        found_high = torch.full_like(eps_low, False, dtype=torch.bool)

        eps = torch.full_like(eps_low, self.init_eps)
        max_l0 = inputs.shape[2]*inputs.shape[3]-1
        for i in range(self.search_steps):
            ...
            adv_inputs_run = self.attack(inputs=inputs, labels=labels, epsilon=eps)
...
def project_L0_box(y, k, lb, ub):
    ...
    p3 = np.sort(np.reshape(p1 - p2, [p2.shape[0], -1]))[np.arange(p2.shape[0]), -k.cpu()]
...
def PGD0_minimal(model: torch.nn.Module,
    ...
    adv_examples = pgd_minimal(inputs=inputs, labels=labels, target_model=target_model)
    return adv_examples
