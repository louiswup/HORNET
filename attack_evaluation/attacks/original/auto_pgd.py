...
class APGDAttack():
    ...
    def __init__(
        ...
        #self.eps_orig = eps.detach().reshape([-1]) + 0.
        self.eps_orig = eps + 0.
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        step_size = alpha * self.eps.to(self.device).detach()
            ...
            with torch.no_grad():
                ...
                if counter3 == k:
                    if self.norm in ['Linf', 'L2']:
                        ...
                        step_size[fl_redtopk] = alpha * self.eps[fl_redtopk]
    ...
    def decr_eps_pgd(self, x, y, epss, iters, use_rs=True):
        ...
        if not use_rs:
            ...
            x_init += L1_projection(x, x_init - x, epss[0])
        eps_target = epss[-1]
        if self.verbose:
...
def apgd_minimal_wrapper(model: nn.Module,
    ...
    for i in range(search_steps):
        ...
        adv_inputs_run = attack(model=model, inputs=inputs, labels=labels, eps=batch_view(eps))
