...
def foolbox_wrapper(attack: partial,
    ...
    if target_model == None:
        ...
        adv_inputs = fb_attack.run(model=fb_model, inputs=inputs, criterion=criterion, target_model=target_model, **epsilon_kwarg)
    return adv_inputs
...
class FoolboxMinimalWrapper:
    ...
    def __init__(self, attack: partial, init_eps: float, search_steps: int, max_eps: Optional[float] = None, batched: bool = False):
        self.attack = attack()
    ...
    def run(self, model: PyTorchModel, inputs: Tensor,
            criterion: Union[TargetedMisclassification, Misclassification], target_model: Optional[nn.Module]=None) -> Tensor:
        ...
        for i in range(self.search_steps):
            if self.batched:
                crit_eps = crit_class(labels)
                adv_inputs_run = self.attack.run(model=model, inputs=inputs, criterion=crit_eps,
                                                 epsilon=batch_view(eps))
            else:
