...
def get_ta_apgd_hornet(threat_model: str, targeted: bool, num_steps: int, num_restarts: int, loss: str, rho: float,
...
def get_ta_fgsm_hornet(init_eps: Optional[float] = None, search_steps: int = minimal_search_steps) -> Callable:
...
def get_ta_pgd_hornet(num_steps: int, alpha: float, random_start: bool,
                       init_eps: Optional[float] = None, search_steps: int = minimal_search_steps) -> Callable:
    init_eps = minimal_init_eps['linf'] if init_eps is None else init_eps
...
def get_ta_pgd_l2_hornet(num_steps: int, alpha: float, random_start: bool, eps_for_division: float,
                          init_eps: Optional[float] = None, search_steps: int = minimal_search_steps) -> Callable:
    ...
    return partial(TorchattacksMinimalWrapper, attack=attack, init_eps=init_eps, search_steps=search_steps, batched=True)
