...
def cleverhans_wrapper(attack: Callable,
    ...
    if target_model == None:
        ...
        adv_examples = attack(model_fn=model, x=inputs, target_model=target_model, **attack_kwargs)
    return adv_examples
...
def cleverhans_minimal_wrapper(attack: Callable,
                               ...
                               batched: bool = True, **kwargs) -> Tensor:
    device = x.device
    batch_size = len(x)
    batch_view = lambda tensor: tensor.view(batch_size, *[1] * (x.ndim - 1))
    ...
    best_eps = torch.full_like(eps_low, float('inf') if max_eps is None else max_eps)
    ...
    for i in range(search_steps):
        if batched:
            ...
            if 'y' in kwargs:
                eps_kwargs['y'] = kwargs['y']
            if 'y_target' in kwargs:
                eps_kwargs['y_target'] = kwargs['y_target']
            adv_inputs_run = attack(model_fn=model_fn, x=x, eps=batch_view(eps), **eps_kwargs)
        else:
